...
如果配置 qBittorrent，请填写你自己的 Web UI 用户名和密码；不要沿用空密码、默认密码或临时初始化密码。部署密钥只使用直接环境变量：在 `.env` 中设置 `MT_TOKEN`、`MT_ENGINE_API_KEY`、可选的 `QBITTORRENT_PASSWORD` 和可选的 `PUSHPLUS_TOKEN`；不要提交真实值。

非 DEBUG 部署必须设置 `MT_ENGINE_API_KEY`。破坏性或会改变状态的接口要求客户端或受保护的反向代理发送 `Authorization: Bearer <key>` 或 `X-MT-ENGINE-Key`；内置 Web UI 会在首次执行受保护操作时提示输入该 key，并保存在浏览器 local storage。只读验收接口不需要。
...
| `MT_ENGINE_API_KEY` | 非 DEBUG 是 | 保护破坏性或会改变状态的接口；客户端或反向代理发送 `Authorization: Bearer <key>` 或 `X-MT-ENGINE-Key` | - |
