...
**M-Team Engine v3.0.0** 包含两大功能：
1. **免费猎手** - 自动追踪 Free/2xFree 限时免费种子
2. **搜索引擎** - 全能种子搜索，支持高级筛选和一键下载

**v3.0.0 新特性：**
- 安全加固：修复 XSS 漏洞，HTTPS 推送，增强参数验证
- 性能提升：并行 API 调用，刷新速度提升 40%
- 可靠性：JSON 解析错误处理，重构速率限制逻辑
- 无障碍：全面支持 ARIA 属性，优化屏幕阅读器体验
- Bug 修复：修复 JavaScript 错误和内存泄漏
- 工业风格切换开关
- 优化移动端体验
...
# ⚠️ 必须填写 NAS 的实际 IP，不能用 localhost！
...
> ⚠️ **重要提示**
>
> 如果 MT-Engine 和 qBittorrent 都通过 Docker 部署在 NAS 上，必须使用 NAS 的实际 IP 地址（如 `192.168.1.100`），**不能用 localhost**！
> 因为每个 Docker 容器的 localhost 指向的是容器自身，无法访问其他容器。
...
# qBittorrent 配置（必须使用 NAS 的实际 IP）
