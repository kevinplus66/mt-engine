...
**M-Team Engine v3.0.2** 包含两大功能：
1. **免费猎手** - 自动追踪 Free/2xFree 限时免费种子
2. **搜索引擎** - 全能种子搜索，支持高级筛选和一键下载

**v3.0.2 新特性：**
- 性能优化：API 调用缓存机制（用户状态 1 小时，分类列表 24 小时）
- 效率提升：缓存有效时 API 调用减少 60%（从 10 次降至 4 次）
- 用户体验：移除下拉刷新和 R 键快捷键，防止误触
- 平板优化：seeder 表格在 769px-1200px 支持横向滚动
