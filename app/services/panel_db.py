...
def init_database():
    ...
    cursor.execute("""
        ...
        ON traffic_stats(source, timestamp)
    """)
...
async def save_panel_stats_batch(
    timestamp: int,
    qb_traffic: Optional[Dict] = None,
    mteam_traffic: Optional[Dict] = None,
    user_stats: Optional[Dict] = None
) -> bool:
    """在同一个事务中保存 PANEL 采样数据"""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if qb_traffic is not None:
            ...
            """, (
                ...
                "qbittorrent",
                qb_traffic["uploaded"],
                qb_traffic["downloaded"],
                qb_traffic.get("upload_speed", 0),
                qb_traffic.get("download_speed", 0),
            ))

        if mteam_traffic is not None:
            cursor.execute("""
                INSERT OR REPLACE INTO traffic_stats
                (timestamp, source, uploaded, downloaded, upload_speed, download_speed)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                timestamp,
                "mteam",
                mteam_traffic["uploaded"],
                mteam_traffic["downloaded"],
                mteam_traffic.get("upload_speed"),
                mteam_traffic.get("download_speed"),
            ))
        ...
        if user_stats is not None:
            ...
            """, (
                timestamp,
                user_stats["share_ratio"],
                user_stats.get("uploaded"),
                user_stats.get("downloaded"),
                user_stats.get("bonus"),
                user_stats.get("seeding_count"),
                user_stats.get("leeching_count"),
                user_stats.get("user_level"),
            ))
        ...
        if conn:
            conn.rollback()
        logger.error(f"批量保存 PANEL 统计失败: {e}")
        return False
    finally:
        if conn:
            conn.close()
